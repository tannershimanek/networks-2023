...
int main(int argc, char* argv[])
{
	...
	printf("\nTS's IPv6 UDP server is ready for client connection...\n");

	for (;;) {
		clientInfoLen = sizeof(clientInfo);
		char buffer[BUFSIZE];
		int recvBytes = recvfrom(sock, buffer, BUFSIZE, 0, (struct sockaddr*)&clientInfo, &clientInfoLen);
		...
		if (recvBytes > 0) {
			...
			sendto(sock, buffer, recvBytes, 0, (struct sockaddr*)&clientInfo, clientInfoLen);
		}
	}
